...
use crate::repository::{
    ai_chat_activity_repository, ai_chat_character_repository, ai_chat_history_repository,
};
...
impl ChatAIUseCase {
    ...
    pub async fn run(
        ...
        let ai_chat_characters = match self
            .ai_chat_character_repository
            .get(&[ai_chat_character_id])
        {
            Ok(ai_chat_characters) => ai_chat_characters,
            Err(ai_chat_character_repository::Error::CharacterNotFound(_)) => {
                return Err(UseCaseError::CharacterNotFound);
            }
        };
        let ai_chat_character = ai_chat_characters
            .first()
            .ok_or(UseCaseError::CharacterNotFound)?;
        ...
        let mut ai_chat_history = match self
            .ai_chat_history_repository
            .get(user_id, ai_chat_character.id())
        {
            Ok(ai_chat_history) => ai_chat_history,
            Err(ai_chat_history_repository::Error::EntryNotFound(_)) => {
                AIChatHistory::with_key(user_id.to_string(), ai_chat_character.id().clone())
            }
        };
        ...
        let user_prompt = self
            .fluent_proxy
            .get_message("ai-chat--user-prompt--body", Some(&fluent_args));
        ...
        let system_prompt = self
            .fluent_proxy
            .get_message("ai-chat--system-prompt--body", Some(&fluent_args));
