...
impl AIChat {
    pub fn new(chat_ai_use_case: Arc<ChatAIUseCase>, fluent_proxy: Arc<FluentProxy>) -> Self {
        Self {
            chat_ai_use_case,
            fluent_proxy,
        }
...
impl CommandRunner for AIChat {
    async fn run(&self, context: &Context, interaction: &CommandInteraction) -> CommandResult {
        ...
        let user_name = interaction
            ...
            .unwrap_or(
                self.fluent_proxy
                    .get_message("ai-chat--user-name--default", None)
                    .to_string(),
            );

        let use_case_result = match self
            .chat_ai_use_case
            .run(&user_id, &user_name, character_id, message, &now)
            .await
        {
            Ok(result) => result,
            Err(use_case::chat_ai_use_case::UseCaseError::ChatCountExceeded) => {
                let error_message = self
                    .fluent_proxy
                    .get_message("ai-chat--error--execution-limit-exceeded", None);
                interaction
                    .create_followup(
                        &context.http,
            ...
            Err(use_case::chat_ai_use_case::UseCaseError::CharacterNotFound) => {
                let error_message = self
                    .fluent_proxy
                    .get_message("ai-chat--error--character-not-found", None);
                interaction
                    .create_followup(
                        &context.http,
                        CreateInteractionResponseFollowup::new().content(error_message),
        ...
        let reference_items_message = match use_case_result.ai_text.web_references() {
            None => self
                .fluent_proxy
                .get_message("ai-chat--response--no-reference", None)
                .to_string(),
            Some(references) => references
                ...
                .map(|reference| {
                    ...
                    self.fluent_proxy
                        .get_message("ai-chat--response--reference-item", Some(&fluent_args))
                })
                .collect::<Vec<_>>()
                .join("\n"),
        };
        ...
        let body = self
            .fluent_proxy
            .get_message("ai-chat--response--body", Some(&fluent_args));

        let is_message_length_exceeded = body.chars().count() > MAX_MESSAGE_LENGTH;
        let result_message = if is_message_length_exceeded {
            self.fluent_proxy
                .get_message("ai-chat--response--message-length-exceeded", None)
                .to_string()
        } else {
            body.to_string()
        };

        let mut followup_response =
            CreateInteractionResponseFollowup::new().content(result_message);
        if is_message_length_exceeded {
            let attachment = CreateAttachment::bytes(body.as_bytes(), "response.md");
...
impl CommandFactory for Factory {
    ...
    fn command_specification(&self) -> CreateCommand {
        let mut character_type_option = CreateCommandOption::new(
            ...
            "character-id",
            self.fluent_proxy
                .get_message("ai-chat--command-option--character-id--description", None),
        )
        .required(true);

        for ai_chat_character in self.ai_chat_character_repository.get_all() {
            character_type_option = character_type_option.add_int_choice(
                ai_chat_character.display_name(),
                ai_chat_character.id().0.try_into().unwrap(),
            ) // TODO: Handle error
        }

        let message_option = CreateCommandOption::new(
            CommandOptionType::String,
            "message",
            self.fluent_proxy
                .get_message("ai-chat--command-option--message--description", None),
        )
        .required(true)
        .min_length(1)
        .max_length(400);
        ...
        CreateCommand::new(self.command_name())
            .description(
                self.fluent_proxy
                    .get_message("ai-chat--command--description", None),
            )
