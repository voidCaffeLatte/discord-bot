...
impl CommandRunner for AIConversation {
    async fn run(&self, context: &Context, interaction: &CommandInteraction) -> CommandResult {
        ...
        let use_case_result = self
            .chat_ai_characters_use_case
            .run(&user_id, &character_ids, theme, &now)
            .await;
        let use_case_result = match use_case_result {
            ...
            Err(chat_ai_characters_use_case::Error::ChatCountExceeded) => {
                let error_message = self
                    .fluent_proxy
                    .get_message("ai-conversation--error--execution-limit-exceeded", None);
                interaction
                    .create_followup(
                        &context.http,
            ...
            Err(chat_ai_characters_use_case::Error::CharacterCountNotEnough) => {
                let error_message = self
                    .fluent_proxy
                    .get_message("i-conversation--error--character-count-not-enough", None);
                interaction
                    .create_followup(
                        &context.http,
                        CreateInteractionResponseFollowup::new().content(error_message),
        ...
        let reference_items_message = match use_case_result.ai_text.web_references() {
            None => self
                .fluent_proxy
                .get_message("ai-conversation--response--no-reference", None)
                .to_string(),
            Some(references) => references
                ...
                .map(|reference| {
                    ...
                    self.fluent_proxy.get_message(
                        "ai-conversation--response--reference-item",
                        Some(&fluent_args),
                    )
                })
                .collect::<Vec<_>>()
        ...
        let response_body = self
            .fluent_proxy
            .get_message("ai-conversation--response--body", Some(&fluent_args));

        let is_message_length_exceeded =
            response_body.chars().count() > Self::MAX_MESSAGE_LENGTH as usize;
        let response_message = if is_message_length_exceeded {
            self.fluent_proxy
                .get_message("ai-conversation--response--message-length-exceeded", None)
                .to_string()
        } else {
        ...
        if is_message_length_exceeded {
            let attachment = CreateAttachment::bytes(response_body.as_bytes(), "response.md");
...
impl CommandFactory for Factory {
    ...
    fn command_specification(&self) -> CreateCommand {
        let mut command_options: Vec<CreateCommandOption> = (1
            ..=AIConversation::MAX_AI_CHAT_CHARACTER_COUNT)
            .map(|index| {
                let fluent_args = fluent_args![
                    "index" => index,
                ];
                ...
                let mut character_type_option = CreateCommandOption::new(
                    ...
                    self.fluent_proxy.get_message(
                        "ai-conversation--command-option--character-id--description",
                        Some(&fluent_args),
                    ),
                )
                .required(index <= 2);
        ...
        command_options.insert(
            ...
            CreateCommandOption::new(
                ...
                "theme",
                self.fluent_proxy
                    .get_message("ai-conversation--command-option--theme--description", None),
            )
            .required(true)
            .min_length(1)
            .max_length(150),
        ...
        CreateCommand::new(self.command_name())
            .description(
                self.fluent_proxy
                    .get_message("ai-conversation--command--description", None),
            )
