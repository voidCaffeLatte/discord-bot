...
impl application::gateway::you_tube::playlist_item_gateway::PlaylistItemGateway
    for PlaylistItemGateway
{
    async fn get_by_playlist_id(
        ...
        let mut results: Vec<PlaylistItem> =
            Vec::with_capacity(Self::VIDEO_COUNT_PER_FETCH * Self::FETCH_COUNT);
        ...
        for _ in 0..Self::FETCH_COUNT {
            ...
            if let Some(page_token) = page_token {
                queries.push(("pageToken", page_token))
            }
            ...
            let items = items.iter().map(|item| {
                let privacy_status = item.status.privacy_status().unwrap_or_else(|| {
                    warn!(
                        "Unsupported privacy type: {}. Use \"Private\" instead.",
                        item.status.privacy_status
                    );
                    PrivacyStatus::Private
                });
                PlaylistItem::new(item.content_details.video_id.clone(), privacy_status)
            });
        ...
        let cache = CachedValue::new(
            results.clone(),
            *at,
            Duration::hours(Self::CACHE_AVAILABLE_HOURS),
        );
