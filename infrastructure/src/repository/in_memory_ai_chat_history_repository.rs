...
impl AIChatHistoryRepository for InMemoryAIChatHistoryRepository {
    fn get(
        ...
        self.ai_chat_histories.get(&(user_id.to_string(), ai_chat_character_id.clone()))
            .map(|entry| entry.clone())
            .ok_or(Error::EntryNotFound(user_id.to_string()))
    }

    fn set(&self, ai_chat_history: AIChatHistory) {
        let key = (ai_chat_history.user_id().to_string(), ai_chat_history.ai_chat_character_id().clone());
